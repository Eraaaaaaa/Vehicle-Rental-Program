...
void addClientFile(const char* filename, struct client c[]) {
    ...
    printf("\n\t~You are adding a NEW client in the file.~\n");
    printf("Please enter the credentials of the new client as follows:\n");
    printf ("\n----------------------------------------------------------\n");
    printf("Client ID: ");
    ...
    fprintf(f3, "%s  %s  %s  %s  %s  %s\n", newClient.client_ID,newClient.name,newClient.surname,
            newClient.passport_ID,newClient.state,newClient.phone_nr);

    fclose(f3);
    printf("You added a new client to the file successfully!\n");
    printf("-----------------------------------------------------\n");
}
