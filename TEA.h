...
void search_by_year(char user[], FILE* f1, FILE* f2, int year)
{
    ...
    do
    {
        ...
        {
            fscanf(f1, "%s %s %d %s %lf %d %f %d", v.plate_nr, v.model, &v.year, v.fuel_type, &v.consumption, &v.seats, &v.daily_price,&v.reservations);
            {
                if (v.year == year)
                {
    ...
    if (found)
    {
        ...
        while (fgets(line, sizeof(line), f1))
        {
            ...
            {
                if (strcmp(v.plate_nr, input) == 0)
                {
                    found = 1;
                    break; // Exit the loop if the plate number is found
                }
            }
        }
        ...
        if (found)
        {
            printf("The selected vehicle with plate number %s, model %s, and daily price %.2f is available.\n", v.plate_nr, v.model, v.daily_price);
        ...
        {
            printf("The plate number you entered is not available.\n");
            printf("\n----------------------------------------\n");
        }
    }
...
void print_clients_by_reservation(FILE* f3, FILE* f2)
{
    ...
    {
fscanf(f3,"%s %s %s %s %[^0-9] %s", c[num_clients].client_ID, c[num_clients].name, c[num_clients].surname, c[num_clients].passport_ID, c[num_clients].state, c[num_clients].phone_nr);
   num_clients++;
    }
